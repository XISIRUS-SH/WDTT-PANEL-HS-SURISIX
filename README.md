...
WDTT Plus can expose an optional Web Panel from the same `wdtt-server` process. The
panel does not create a second database or duplicate the transport layer.
...
The same value may be supplied with `WDTT_PANEL_LISTEN`. Authentication uses the
existing main administrator password. The browser receives an HttpOnly session
cookie; the main password is not stored in localStorage.

The panel uses the existing `dbMutex` and `executeAdminCommand(..., wgDev, true)` so
live WRAP/WireGuard effects remain in the server's existing control path.

Linux release installer:

```bash
curl -fsSL https://raw.githubusercontent.com/XISIRUS-SH/WDTT-PANEL-HS-SURISIX/main/installer/install.sh | sudo bash
```

The installer refuses to create a second WDTT service, checks for an existing
`wdtt.service`, detects occupied panel ports, backs up the current binary, verifies
the release SHA256, and uses a systemd drop-in for `WDTT_PANEL_LISTEN`.

## GitHub upload

Upload the **contents of this archive** directly into the repository root. Do not add another `WDTT-Plus-15/` wrapper directory. `README.md`, `go.mod`, `server.go`, `panel.go`, `panelui/`, and the deployment scripts belong at the root.

## VPS install

```bash
curl -fsSL https://raw.githubusercontent.com/XISIRUS-SH/WDTT-PANEL-HS-SURISIX/main/install.sh | sudo bash
```

The installer clones the repository, verifies the expected root layout, ensures Go 1.25+, runs `go test ./...` and `go vet ./...`, builds the actual server, installs a dedicated systemd unit, and shows recent/live logs.
