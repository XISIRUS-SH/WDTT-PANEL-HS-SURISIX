...
func main() {
	...
	panelListenDefault := os.Getenv("WDTT_PANEL_LISTEN")
	if panelListenDefault == "" {
		panelListenDefault = "127.0.0.1:8787"
	}
	panelListen := flag.String("panel-listen", panelListenDefault, "HTTP адрес Web Panel; `-` отключает панель")
	flag.Parse()
	...
	panelClose, err := startWebPanel(ctx.Done(), *panelListen, wgDev)
	if err != nil {
